...
public class Main {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        Thread a = new Thread(() -> {
            char letter = 'a';
            maxString(queue1, letter);
        });
        a.start();

        Thread b = new Thread(() -> {
            char letter = 'b';
            maxString(queue2, letter);
        });
        b.start();

        Thread c = new Thread(() -> {
            char letter = 'c';
            maxString(queue3, letter);
        });
        c.start();
    ...
    public static void maxString(BlockingQueue<String> queue, char letter) {
        System.out.println("Максимальное количество символов " + letter + " - " + maxLetters(queue, letter));
    }
}